...
export function isEditMode(app) {
  ...
  if (typeof app.$storyblok !== 'undefined') {
    ...
    const spaceID = app.$storyblok.getParam(
      encodeURI('_storyblok_tk[space_id]')
    )
    const timestamp = app.$storyblok.getParam(
      encodeURI('_storyblok_tk[timestamp]')
    )
    ...
    const controlToken = crypto
      .createHash('sha1')
      .update(`${spaceID}:${app.$storyapi.accessToken}:${timestamp}`)
      .digest('hex')

    // check if the controlToken is equal to the validation token passed as param
    // and if timestamp is in the lates 60 minutes.
    if (
      app.$storyblok.getParam(encodeURI('_storyblok_tk[token]')) ===
        controlToken &&
      app.$storyblok.getParam(encodeURI('_storyblok_tk[timestamp]')) >
